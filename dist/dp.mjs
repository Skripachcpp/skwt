    ...
    for (let tsxPath in tsx) {
        ...
        for (let importMatched of importsMatched) {
            ...
            if (importPathArray?.[0] == '~') {
                ...
                if (importPathArray?.[1] != 'components')
                    continue;
...
async function go() {
    ...
    function initChildren(tsxElement, level = 0) {
        ...
        if (tsxElement.parentLevels == null)
            tsxElement.parentLevels = [];
        if (!tsxElement.parentLevels.includes(level))
            tsxElement.parentLevels.push(level);
        ...
        for (let imp of (tsxElement.imports || [])) {
            ...
            if (impCmp == null)
                impCmp = tsx[importPath + '/index.tsx'];
            if (impCmp) {
    ...
    function checkPaths(componentElement) {
        if (!componentElement?.parents?.length)
            return;
